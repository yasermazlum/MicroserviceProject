...
namespace EventBus.Base.SubManagers
{
    public class InMemoryEventBusSubscriptionManager : IEventBusSubscriptionManager
    {
        ...
        {
            if(!HashSubscriptionForEvent(eventName))
        ...
        public bool HashSubscriptionForEvent<T>() where T : IntegrationEvent
        {
            var key = GetEventKey<T>();
            return HashSubscriptionForEvent(key);
        }

        public bool HashSubscriptionForEvent(string eventName) => _handlers.ContainsKey(eventName);
        ...
        private void RemoveHandler(string eventName, SubscriptionInfo subsRemove)
        {
            if(subsRemove != null)
            {
                _handlers[eventName].Remove(subsRemove);
